#!/usr/bin/env python3
"""Evaluate VN3K retrieval with board-generated text embeddings.

Use this after running the text context binary on RB3 for a smoke subset or the
full VN3K query set. The script reads qnn-net-run `Result_*/output_0.raw` files,
maps them to pids through the prepared text `manifest.csv`, encodes image
gallery embeddings with the FP32 PyTorch model, and reports text-isolation
retrieval: image FP32 + text QNN board.
"""
...
def _load_board_text_embeddings(
    output_dir: Path,
    query_input_dir: Path,
    output_name: str,
    expected_dim: int,
    max_captions: int,
) -> tuple[torch.Tensor, torch.Tensor]:
    manifest_rows = _read_manifest(query_input_dir / "manifest.csv")
    ...
    if max_captions:
        qnn_paths = qnn_paths[:max_captions]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Merged FP32 model dir used to encode image gallery.",
    )
    parser.add_argument(
        "--dataset-root",
        type=Path,
...
def main() -> None:
    ...
    test_img_set, _ = _build_test_sets(config, dataset_root, tokenizer)
    ...
    image_fp32, _, image_ids, _ = _encode_images(
        model, None, img_loader, device, tuple(config.aug.img.size), args.max_images
    )
    ...
    print(
        ...
        f"image FP32 {tuple(image_fp32.shape)}"
    )
    metrics = _metrics(text_board, image_fp32, text_ids, image_ids, normalize=False)
    _print_table("BOARD TEXT-INT8 (image FP32 + text QNN board)", metrics)
    ...
    print(f"\nBOARD TEXT GATE T2I R@1 >= {args.gate_r1}: {metrics['t2i']['R1']:.2f} -> {verdict}")
    ...
    summary = {
        ...
        "combo": "board_text_int8_image_fp32",
        "metrics": metrics,
