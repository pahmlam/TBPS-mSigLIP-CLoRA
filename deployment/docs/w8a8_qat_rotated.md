...
where $z_t = \text{teacher.encode\_image}(x)$, $z_s^{q}$/$z_s^{c}$ are the fake-quant / clean student embeddings, $\lambda = w_m = 0.05$, $w_c = 1.0$. The clean term prevents the student from drifting away from the teacher when quant noise is off.
