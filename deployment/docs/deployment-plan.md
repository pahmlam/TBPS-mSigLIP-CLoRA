...
After the AI Hub attempts now consolidated in `deployment/docs/journal/[deploy-master].md`, the root cause is confirmed and resolved:
...
| Path | Purpose |
|------|---------|
| `deployment/docs/journal/[deploy-master].md` | **Canonical deploy journal** for AI Hub jobs, QNN/QDQ fidelity, RB3 runtime, artifacts, decisions, and next plans |
| `deployment/docs/journal/` | Archive of older dated deploy journals plus demo-system logs |
| `deployment/docs/aihub-experiments.md` | Legacy redirect; do not write new entries |
...
| `README.md` Deployment Status section | Pipeline quick-reference + AI Hub commands now consolidated at the repository root |
