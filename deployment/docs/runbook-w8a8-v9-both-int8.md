# Runbook — W8A8 v9 Both-INT8 (Vision + Text) trên RB3 Gen2 (HTP v68)
...
>
> **Runbook này dùng vision path v9 và text path split-encoder cho số chốt**. v8 learned rotation là mốc phương pháp đã thắng v6/v7 (vision QDQ T2I R@1 `50.85`); v9 là cùng phương pháp nhưng tăng calibration/steps rotation và QAT epochs để nâng số board cuối.
...
## 0A. Setup từ đầu trên máy local/cuda box (làm 1 lần)

Làm toàn bộ phần này trên máy submit job, **không làm trên RB3**. RB3 chỉ cần nhận context binary, raw input smoke test, token-embedding table, và script board-run ở các phần sau.

### 0A.1 Vào repo và tạo virtual environment
...
Nếu đã có `venv/`, chỉ cần `source venv/bin/activate`. Các bước local dùng `python` trong venv; các bước board nếu cần Python thì dùng Python có sẵn trên RB3.

### 0A.2 Cài thư viện Python

```bash
python -m pip install -r requirements.txt
```

Kiểm tra nhanh các package deployment quan trọng:
...
print("onnxconverter_common", getattr(onnxconverter_common, "__version__", "unknown"))
print("qai_hub", getattr(hub, "__version__", "unknown"))
PY
```

`qai_hub 0.48.0` là phiên bản đang dùng ổn trong repo này. QNN/QAIRT/SNPE không nằm trong `requirements.txt`; đó là SDK/CLI của Qualcomm và được kiểm ở bước board/SDK riêng.

### 0A.3 Cấu hình Qualcomm AI Hub token

Vào Qualcomm AI Hub / Workbench → Settings / Account → API token, rồi cấu hình default profile:
...
Không dùng `--profile` phụ cho runbook này, trừ khi bạn cũng sửa mọi script submit để dùng profile đó. Các script như `submit_qaihub_quantize_compile.py` mặc định đọc profile mặc định.

### 0A.4 Smoke test kết nối AI Hub
...
### 0A.5 Chuẩn bị VN3K trong repo

Các script nhận `--dataset-root VN3K` và tự resolve được cả hai dạng: truyền trực tiếp thư mục `VN3K/`, hoặc truyền thư mục cha có chứa `VN3K/`. Layout tối thiểu phải có:

```text
VN3K/
├── data_captions_vn3k.json
└── imgs/
```

Nếu dataset nằm ngoài repo, tạo symlink để lệnh trong runbook giữ nguyên:

```bash
ln -s /path/to/VN3K VN3K
```

Kiểm tra nhanh:

```bash
test -f VN3K/data_captions_vn3k.json
test -d VN3K/imgs
python - <<'PY'
import json
...
root = Path("VN3K")
ann = json.loads((root / "data_captions_vn3k.json").read_text(encoding="utf-8"))
splits = {}
for row in ann:
    splits[row.get("split", "")] = splits.get(row.get("split", ""), 0) + 1
print("VN3K annotations:", len(ann), splits)
print("imgs dir:", root / "imgs")
PY
```

Sau khi check layout, chạy PART P để sinh raw inputs cho vision/text. Vision calibration có thể tái dùng `VISION_CALIB_ID=d7jzjy1m2`; nếu dataset AI Hub hết hạn, chạy lại P2 rồi upload bằng `upload_qaihub_calibration_dataset.py`.

### 0A.6 Nguyên tắc khi submit job
