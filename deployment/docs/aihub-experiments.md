...
| 15 | 2026-05-27 | `jpr9v62vp` | `vision_onnx_static/` QDQ model from `submit_quantize_job` | Python API compile/link | Static input `[1,3,256,256]`, `--quantize_io`, real calibration `d7x5gzne9` | ✅ | Compile log has no `--preserve_io_datatype`; `qairt-converter` completed, QNN DLC asset `mq389x29m` uploaded, and local `vision_encoder_calib500.bin` was downloaded (~90 MB). Warnings remain: some ops fallback to float and repeated signed-offset warnings. | Real-calibration API path now produces a binary. Next gate is on-board RB3 execution + QNN-vs-PyTorch fidelity; do not compile text until this binary passes. |
