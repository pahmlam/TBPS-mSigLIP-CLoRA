...
9. **Compile pass is not fidelity pass** — Job `jpr9v62vp` produced a real-calibration binary that runs on RB3 HTP, but on-board comparison still failed (`cosine_l2_mean = 0.1300`, lower than dummy-cal `0.1727`). After any successful compile, the next mandatory gate is QNN-vs-PyTorch fidelity on the same raw inputs.
...
| 15 | 2026-05-27 | `jpr9v62vp` | `vision_onnx_static/` QDQ model from `submit_quantize_job` | Python API compile/link | Static input `[1,3,256,256]`, `--quantize_io`, real calibration `d7x5gzne9` | ⚠️ | Compile log has no `--preserve_io_datatype`; `qairt-converter` completed, QNN DLC asset `mq389x29m` uploaded, and local `vision_encoder_calib500.bin` was downloaded (~90 MB). On RB3 the binary ran, but fidelity failed: `cosine_l2_mean = 0.1300`, no NaN/Inf. | Real-calibration API path fixes compile/link and HTP runtime, but not embedding correctness. Next gate is comparing the QDQ ONNX against PyTorch to locate whether drift comes from PTQ/calibration or QNN runtime/I/O. |
