...
from msiglip.utils.logger import setup_logging
from msiglip.utils.visualize_test import prepare_prediction_for_wandb_table
...
def run(config: DictConfig) -> None:
    ...
    if config.logger.logger_type == "wandb":
        ...
        data = [
            [
                wandb_module.Image(data) if isinstance(data, Image.Image) else data
                for data in row
            ]
            for row in data
        ]
    ...
    if wandb_module is not None and wandb_module.run:
        wandb_module.finish()


@hydra.main(version_base="1.3", config_path=CONFIG_DIR)
